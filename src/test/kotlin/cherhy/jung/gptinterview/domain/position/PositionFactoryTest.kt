...
internal class PositionFactoryTest: FeatureSpec({
    feature("개발자 타입을 인자로 받으면 스펙에 맞는 하위 클래스 로 반환 한다.") {
        ...
        scenario("DATA_ANALYSIS 를 받으면 DataAnalysis 타입의 스펙과 같아 진다.") {
            val dataAnalysis = PositionFactory.valueOf(DATA_ANALYSIS) as DataAnalysis
        ...
        scenario("DATA_ENGINEER 를 받으면 DataEngineer 타입의 스펙과 같아 진다.") {
            val dataEngineer = PositionFactory.valueOf(DATA_ENGINEER) as DataEngineer
        ...
        scenario("BACKEND 를 받으면 BackendDeveloper 타입의 스펙과 같아 진다.") {
            val backendDeveloper = PositionFactory.valueOf(BACKEND) as BackendDeveloper
        ...
        scenario("FRONTEND 를 받으면 FrontendDeveloper 타입의 스펙과 같아 진다.") {
            val frontendDeveloper = PositionFactory.valueOf(FRONTEND) as FrontendDeveloper
        ...
        scenario("ANDROID 을 받으면 AndroidDeveloper 타입의 스펙과 같아 진다.") {
            val androidDeveloper = PositionFactory.valueOf(ANDROID) as AndroidDeveloper
        ...
        scenario("IOS 를 받으면 IosDeveloper 타입의 스펙과 같아 진다.") {
            val iosDeveloper = PositionFactory.valueOf(IOS) as IosDeveloper
        ...
        scenario("AI 을 받으면 AiDeveloper 타입의 스펙과 같아 진다.") {
            val aiDeveloper = PositionFactory.valueOf(AI) as AiDeveloper
        ...
        scenario("BLOCKCHAIN 을 받으면 BlockchainDeveloper 타입의 스펙과 같아 진다.") {
            val blockchainDeveloper = PositionFactory.valueOf(BLOCKCHAIN) as BlockchainDeveloper
        ...
        scenario("DBA 를 받으면 DatabaseAdministrator 타입의 스펙과 같아 진다.") {
            val databaseAdministrator = PositionFactory.valueOf(DBA) as DatabaseAdministrator
